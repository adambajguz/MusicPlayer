...
{
    public class GetSongsByDateDescending
    {
        ...
        public class HandlerList : IQueryHandler<Query, List<Result>>
        {
            ...
            {
                var result = await _uow.SongRepository.Query().Select(x => new Result(x)).OrderByDescending(y => y.DBCreationDate).ToListAsync();
