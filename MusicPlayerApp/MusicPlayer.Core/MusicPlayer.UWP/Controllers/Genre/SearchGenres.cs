...
namespace MusicPlayer.UWP.Controllers.Genre
{
    public class SearchGenres
    {
        ...
        public class HandlerList : IQueryHandler<Query, List<Result>>
        {
            ...
            {
                var result = await _uow.GenreRepository.Query().Where(x => x.Name.ToUpper().Contains(query.Name.ToUpper())).Select(x => new Result(x)).OrderBy(y => y.Name).ToListAsync();
