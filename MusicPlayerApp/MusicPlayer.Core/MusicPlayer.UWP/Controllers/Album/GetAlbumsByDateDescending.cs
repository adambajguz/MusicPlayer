...
{
    public class GetAlbumsByDateDescending
    {
        ...
        public class HandlerList : IQueryHandler<Query, List<Result>>
        {
            ...
            {
                var result = await _uow.AlbumRepository.Query().Select(x => new Result(x)).OrderByDescending(y => y.DBCreationDate).ToListAsync();
