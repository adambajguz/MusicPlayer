...
namespace MusicPlayer.UWP.Controllers.Artist
{
    public class SearchArtists
    {
        ...
        public class HandlerList : IQueryHandler<Query, List<Result>>
        {
            ...
            {
                var result = await _uow.ArtistRepository.Query().Where(x => x.Name.ToUpper().Contains(query.Name.ToUpper())).Select(x => new Result(x)).OrderBy(y => y.Pseudonym).ToListAsync();
