...
{
    public class GetPlaylistsByDateDescending
    {
        ...
        public class HandlerList : IQueryHandler<Query, List<Result>>
        {
            ...
            {
                var result = await _uow.PlaylistRepository.Query().Select(x => new Result(x)).OrderByDescending(y => y.DBCreationDate).ToListAsync();
